...
(function(glob) {
	...
	TextStatistics.prototype.smogIndex = function(text) {
		text = text ? cleanText(text) : this.text;
		return Math.round(1.043 * Math.sqrt((this.wordsWithThreeSyllables(text).longWordCount * (30 / this.sentenceCount(text))) + 3.1291)*10)/10;
	};
	...
	TextStatistics.prototype.sentences = function(text) {
		text = text ? cleanText(text) : this.text;

		return sentences = text.match(/\(?[^\.\?\!]+[\.!\?]\)?/gim);
	}
		...
		text.split(/\s+/).forEach(function (word) {
			...
			if (!word.match(/^[A-Z]/) || countProperNouns) {
				if (self.syllableCount(word) > 2) {
					// Strip () and [] brackets from complex words
					longWords.push(word.replace(/[\(\)\[\]']+/g,'').replace(/\./g, ''));
					longWordCount++;
				}
			}
		});
	...
	TextStatistics.prototype.percentageWordsWithThreeSyllables = function(text, countProperNouns) {
		...
		return (this.wordsWithThreeSyllables(text,countProperNouns).longWordCount / this.wordCount(text)) * 100;
	};
